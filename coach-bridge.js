// coach-bridge.js — runs in the MAIN world (see manifest "world": "MAIN").
//
...
// scraping, no Openings-Explorer contamination), and forwards it to the
// isolated-world coach via window.postMessage.
//
// It only READS and only forwards moves. Whether those moves are turned into
// live hints is decided by coach-ui.js, which suppresses real-time help in
...
(function () {
  ...
  function tick() {
    ...
    last = sig;
    try {
      window.postMessage({ __openingCoach: 'moves', sans: data.sans, playingAs: data.playingAs }, '*');
    } catch {}
  }
