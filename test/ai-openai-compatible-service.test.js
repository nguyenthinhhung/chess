...
test('explainMove parses a successful chat-completions reply', async () => {
  ...
  const result = await explainMove(data, { apiKey: 'k', baseUrl: 'https://api.example.com/v1', fetchImpl: fakeFetch(reply) });
...
test('explainMove posts to <baseUrl>/chat/completions with a bearer token and json_object format', async () => {
  ...
  await explainMove(data, { apiKey: 'sk_test', model: 'test-model', baseUrl: 'https://api.example.com/v1', fetchImpl });
  assert.equal(seenUrl, 'https://api.example.com/v1/chat/completions');
  assert.equal(seenOpts.headers.Authorization, 'Bearer sk_test');
